...
class Mesh {
  Mesh({List<Vector3>? vertices, List<Offset>? texcoords, List<Polygon>? indices, List<Color>? colors, this.texture, Rect? textureRect, this.texturePath, Material? material, this.name}) {
    ...
    // PATCHED (see PATCH_NOTES.md): one averaged normal per (face, corner)
    // instance, computed once here rather than as a flat per-face normal
    // recomputed every render frame -- see `_computeCornerNormals`.
    this.cornerNormals = _computeCornerNormals(this.vertices, this.indices);
  }
  ...
  String? name;
  late List<Vector3> cornerNormals;
}
...
Future<List<Mesh>> loadObj(String fileName, bool normalized, {bool isAsset = true}) async {
  ...
  String? objectlName;
  String? groupName;
  // PATCHED (see PATCH_NOTES.md): `package:path`'s default top-level
  // functions use platform-style separators (backslash on Windows), but a
  // Flutter asset-bundle key is always forward-slash regardless of host
  // OS. On Windows, joining with the platform style produced a path that
  // matched no real asset -- `rootBundle.loadString` threw, and the
  // caller (`loadMtl`) silently swallows that into an empty material map,
  // so every asset-bundled `.obj` with a `mtllib` line silently fell back
  // to the default (untextured, dim) material with no error anywhere.
  // Force posix-style joining specifically for the asset-bundle case;
  // `isAsset: false` (a real filesystem path) still wants the platform's
  // own separator, which the plain top-level functions already give.
  String basePath = isAsset ? path.posix.dirname(fileName) : path.dirname(fileName);
  ...
  for (var line in lines) {
    ...
    switch (parts[0]) {
      case 'mtllib':
        ...
        // PATCHED: see the basePath comment above.
        final mtlFileName = isAsset ? path.posix.join(basePath, parts[1]) : path.join(basePath, parts[1]);
...
/// PATCHED (see PATCH_NOTES.md). Faces meeting at an angle sharper than
/// this are treated as a hard/flat edge (kept crisp, e.g. a cube's 90°
/// corners or a cylinder's flat-cap-to-round-side seam); faces within it
/// blend into each other's average (a cylinder/cone/sphere's actually-
/// curved surface) -- the same "auto-smooth angle" concept modeling tools
/// like Blender use for "shade smooth".
const double _smoothAngleThresholdDeg = 40.0;

/// One averaged vertex normal per (face, corner) instance -- i.e. length
/// `indices.length * 3`, indexed as `faceIndex * 3 + cornerIndex` (0/1/2
/// for vertex0/vertex1/vertex2 of that face) -- rather than one normal per
/// unique vertex POSITION, since the same physical vertex can legitimately
/// need a different normal depending on which face is asking (a cube
/// corner shared by 3 faces should shade flat/differently for each of
/// them; a cylinder's round-side vertices should blend together).
List<Vector3> _computeCornerNormals(List<Vector3> vertices, List<Polygon> indices) {
  final int faceCount = indices.length;
  final List<Vector3> faceNormals = List<Vector3>.generate(faceCount, (i) {
    final p = indices[i];
    if (p.vertex0 >= vertices.length || p.vertex1 >= vertices.length || p.vertex2 >= vertices.length) {
      return Vector3(0, 0, 1);
    }
    final n = (vertices[p.vertex1] - vertices[p.vertex0]).cross(vertices[p.vertex2] - vertices[p.vertex0]);
    return n.length2 > 0 ? n.normalized() : Vector3(0, 0, 1);
  });

  final Map<int, List<int>> facesTouchingVertex = <int, List<int>>{};
  for (int i = 0; i < faceCount; i++) {
    final p = indices[i];
    for (final v in [p.vertex0, p.vertex1, p.vertex2]) {
      (facesTouchingVertex[v] ??= <int>[]).add(i);
    }
  }

  final double cosThreshold = math.cos(_smoothAngleThresholdDeg * math.pi / 180.0);
  final List<Vector3> corners = List<Vector3>.filled(faceCount * 3, Vector3.zero(), growable: false);
  for (int i = 0; i < faceCount; i++) {
    final p = indices[i];
    final Vector3 thisNormal = faceNormals[i];
    final List<int> vertexIds = [p.vertex0, p.vertex1, p.vertex2];
    for (int corner = 0; corner < 3; corner++) {
      final List<int>? touching = facesTouchingVertex[vertexIds[corner]];
      final Vector3 sum = Vector3.zero();
      if (touching != null) {
        for (final fi in touching) {
          if (faceNormals[fi].dot(thisNormal) >= cosThreshold) {
            sum.add(faceNormals[fi]);
          }
        }
      }
      corners[i * 3 + corner] = sum.length2 > 0 ? sum.normalized() : thisNormal;
    }
  }
  return corners;
}
