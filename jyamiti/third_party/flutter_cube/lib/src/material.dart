...
Future<MapEntry<String, Image>?> loadTexture(Material? material, String basePath, {bool isAsset = true}) async {
  ...
  while (dirList.length > 0) {
    // PATCHED (see PATCH_NOTES.md and the matching fix in mesh.dart): same
    // platform-vs-asset-bundle separator mismatch -- force posix-style
    // joining for the asset-bundle case.
    fileName = isAsset ? path.posix.join(basePath, path.posix.joinAll(dirList)) : path.join(basePath, path.joinAll(dirList));
    try {
