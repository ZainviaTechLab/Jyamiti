    ...
    if (o.lighting) {
      ...
      for (int i = 0; i < indexCount; i++) {
        ...
        if (renderIndices[indexOffset + i] != null) {
          ...
          // PATCHED (see PATCH_NOTES.md): each corner gets its own
          // angle-thresholded averaged normal instead of one flat normal
          // shared by the whole triangle -- this is what lets curved
          // geometry (sphere/cylinder/cone) shade smoothly across face
          // seams while genuinely sharp edges (a cube's corners) stay
          // crisp.
          final Vector3 normalA = o.mesh.cornerNormals[i * 3 + 0].clone()
            ..applyMatrix4(normalTransform)
            ..normalize();
          final Vector3 normalB = o.mesh.cornerNormals[i * 3 + 1].clone()
            ..applyMatrix4(normalTransform)
            ..normalize();
          final Vector3 normalC = o.mesh.cornerNormals[i * 3 + 2].clone()
            ..applyMatrix4(normalTransform)
            ..normalize();
          ...
          renderColors[vertexOffset + p.vertex0] = light.shading(viewPosition, a, normalA, material).value;
          renderColors[vertexOffset + p.vertex1] = light.shading(viewPosition, b, normalB, material).value;
          renderColors[vertexOffset + p.vertex2] = light.shading(viewPosition, c, normalC, material).value;
        }
      }
    } else {
