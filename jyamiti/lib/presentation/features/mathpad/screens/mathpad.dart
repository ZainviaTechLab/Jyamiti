    ...
    return RepaintBoundary(
      ...
      child: Stack(
        children: [
          ...
          // `_OnCanvasCameraPreview`'s own doc comment for the controller
          // lifecycle.
          ListenableBuilder(
            listenable: _recordingService,
            builder: (context, _) {
...
/// `CameraController`: creates one the moment [active] turns true, disposes
/// it the moment [active] turns false (including if this widget itself
/// gets torn down mid-recording, e.g. leaving the page), so it never
/// outlives exactly the window a recording actually needs it for.
///
/// Deliberately positioned/cropped/bordered to match the picture-in-picture
/// box the OTHER two `CameraEncodeMode`s produce via ffmpeg's overlay
/// filter (`crop=ih:ih,scale=240:240`, white border, top-right, 24px
/// inset) -- see `_encodeSegment`'s filter graph in
/// `mathpad_recording_service.dart` -- so a recording looks the same
/// regardless of which mode made it.
...
class _OnCanvasCameraPreviewState extends State<_OnCanvasCameraPreview> {
  ...
  Future<void> _start() async {
    ...
    try {
      ...
      // `active` may have already flipped back off while `availableCameras()`
      // was in flight (a very quick start/stop, or the recording ended
      // almost immediately) -- don't bother opening the device at all in
      // that case, matching the "camera trouble/timing never blocks the
      // recording" spirit of every other camera path in this app, just
      // applied to "don't bother" instead of "don't fail".
      ...
      // No camera, permission denied, `camera_windows` failure, etc. --
      // this preview just shows nothing rather than an error; the
      // recording itself is entirely unaffected either way, same as
      // every other camera failure path in this app only ever costing
      // the camera, never the recording.
    } finally {
      _initializing = false;
    }
  }
